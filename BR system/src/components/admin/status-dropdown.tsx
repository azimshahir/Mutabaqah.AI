...
export function StatusDropdown({ applicationId, currentStatus }: StatusDropdownProps) {
  ...
  const handleStatusChange = async (newStatus: FinancingStatus, reason?: string) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        // Check for auto-disbursement success
        if ('autoDisbursed' in result && result.autoDisbursed) {
          toast.success(result.message || 'Application approved and auto-disbursed!')
  ...
  const getAvailableActions = () => {
    switch (currentStatus) {
      ...
      case 'approved':
        return [] // Auto-disbursement enabled - no manual actions needed
      case 'rejected':
        return [
          {
